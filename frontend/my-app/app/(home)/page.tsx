...
const HomePage: React.FC = () => {
  ...
  /**
   * Query lấy danh sách sản phẩm bán chạy
  ...
  /**
   * Query lấy danh sách sản phẩm được đề xuất riêng cho người dùng
  ...
  const {
    ...
    skip: !user?.user_id, // Bỏ qua query nếu người dùng chưa đăng nhập
    onError: (error) => {
  ...
  const forYouProducts = dataForYou?.products?.data || [];

  /**
   * Hiển thị trạng thái đang tải (loading) với hiệu ứng skeleton
   * Hiển thị khi bất kỳ một trong ba query đang thực hiện
   */
  if (loadingNewest || loadingBestSeller || loadingForYou) {
    return (
    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-12 pt-3 pb-3">
        ...
        <div className="col-span-10 bg-white rounded-lg shadow-sm p-4">
          ...
          {/* Phần DÀNH CHO BẠN - hiển thị sản phẩm được đề xuất cá nhân hóa */}
          <div className="flex items-center mb-4">
